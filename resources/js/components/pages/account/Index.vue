<template>
  <app-card title="All <b>Account Types</b>" body-padding="0">
    <template #actions>
      <app-btn-link route-name="account.create" background="bibaabo">Add New</app-btn-link>
    </template>

    <app-table-sortable
      :columns="columns"
      :rows="rows"
      :searchUrl="'/admin/account/get-data/'"
    >
      <template #rowData="{ row }">
        ...
        <td width="100">
          <app-actions
            ...
            :actions="{
              edit: { name: 'account.edit', params: { id: row.id } },
              delete: row.id,
            }"
          ></app-actions>
        </td>
...
export default {
  ...
  methods: {
    ...
    reset() {
      axios.get("/admin/account").then((response) => {
