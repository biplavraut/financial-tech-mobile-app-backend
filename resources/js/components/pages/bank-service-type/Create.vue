<template>
  <app-card :title="edit ? 'Edit ' + form.title : 'Add New <b>Bank Service Type</b>'">
    <!-- <template #actions>
      <app-btn-link route-name="banner.create">Add New</app-btn-link>
    </template> -->
    <form @submit.prevent="this.edit ? this.updateData() : this.saveBankServiceType()">
      <div class="row">
        ...
        <div class="col-sm-9 col-md-10">
            ...
            <div class="col-md-6">
              <div class="form-group">
                      ...
                      <select class="form-control" v-model="form.serviceType">
                        ...
                        <option v-for="option in serviceTypes" :value="option.id">{{ option.name }}</option>
                      </select>
                    </div>
              <!-- <input-select
          ...
          <div class="row">
            ...
            <div class="col-md-6">
                <div class="card">
                  <div class="card-body">
                    <h4 class="card-title">Bank Service Type Attributes</h4> 
                     <table>
                    <thead>
                        <tr>
                            <th>SN</th>
  ...
  methods: {
    ...
    async updateData() {
      try {
        ...
        this.$router.push({ name: 'bankServiceType.list', params: { bank: this.form.bank }, });
      } catch (error) {
        console.log(error);
    ...
    async getData() {
      ...
      let { bank, title, slug, image, display, description, serviceType, attributes } = data.data;
    ...
    async getServiceTypes() {
      let serviceTypes = await this.serviceTypeModel.getAll('bank');
      //console.log(accountTypes);
      this.serviceTypes = serviceTypes.data.map((item) => {
        return {
          id: item.id,
          name: item.title,
        };
