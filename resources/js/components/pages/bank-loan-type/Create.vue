<template>
  <app-card :title="edit ? 'Edit ' + form.title : 'Add New <b>Bank Loan Type</b>'">
    <!-- <template #actions>
      <app-btn-link route-name="banner.create">Add New</app-btn-link>
    </template> -->
    <form @submit.prevent="this.edit ? this.updateData() : this.saveBankLoanType()">
      <div class="row">
        ...
        <div class="col-sm-9 col-md-10">
          ...
          <div class="row">
            ...
            <div class="col-md-6">
              <div class="form-group">
                      <label>Main Loan Type</label>
                      <select class="form-control" v-model="form.loanType">
                        <option disabled value="">Please select an option</option>
                        <option v-for="option in loanTypes" :value="option.id">{{ option.name }}</option>
                      </select>
                    </div>
            </div>
          </div>
          <div class="row">
            ...
            <div class="col-md-6">
                <div class="card">
                  <div class="card-body">
                    <h4 class="card-title">Bank Loan Type Attributes</h4> 
                     <table>
                    <thead>
                        <tr>
                            <th>SN</th>
  ...
  methods: {
    ...
    async updateData() {
      try {
        ...
        this.$router.push({ name: 'bankLoanType.list', params: { bank: this.form.bank }, });
      } catch (error) {
        console.log(error);
    ...
    async getData() {
      ...
      let { bank, title, slug, image, display, description, loanType, attributes } = data.data;
    ...
    async getLoanTypes() {
      let loanTypes = await this.loanTypeModel.getAll('bank');
      this.loanTypes = loanTypes.data.map((item) => {
        return {
          id: item.id,
          name: item.title,
        };
